...
//belle++;	
  ...
  else
  {
	  ...
	  IPerr = HepSymMatrix(3,0);
		...
		std::vector<Particle> kstar_cand;
		...
		Ptype ptype_kstar_minus_cand("K*-");
		Ptype ptype_kstar_plus_cand("K*+");
...
/*	  for(std::vector<Particle>::iterator i = tpi_0.begin(); i != tpi_0.end(); i++)
	...
	//			*status = 1;
	  ...
	  if (frecB.tag_id() == -521) {
		  combination(kstar_plus,ptype_kstar_plus_cand,tk_plus,tpi_0);
		  combination(kstar_plus,ptype_kstar_plus_cand,tk_short,tpi_plus);
	  }
	  else if (frecB.tag_id() == 521){
		  combination(kstar_minus,ptype_kstar_minus_cand,tk_minus,tpi_0);
		  combination(kstar_minus,ptype_kstar_minus_cand,tk_short,tpi_minus);
	  }
...
//		  continue;
	  ...
	  float ek = 0 ;
	  ///////////////////////////////////k/////////////////////////////////////////////
	  for(std::vector<Particle>::iterator i5 =kstar_plus.begin(); i5 != kstar_plus.end(); i5++)
	  {
		  Particle Bsig((*i5).momentum().p(), ptype_B_plus_cand);
		  Bsig.relation().append((*i5));
		  B_cand1.push_back(Bsig);
	  }
	  for(std::vector<Particle>::iterator i6 =kstar_minus.begin(); i6 != kstar_minus.end(); i6++)
	  ...
	  for(std::vector<Particle>::iterator i =B_cand1.begin(); i != B_cand1.end(); i++)
	  {
		 //////
		  ...
		  Particle s1 = (*i);
		  chi1 = bestcandchi(s1);
		  for(std::vector<Particle>::iterator j=i+1; j != B_cand1.end(); j++)
		  {
			  Particle s2 = (*j);
			  chi2 = bestcandchi(s2);
	  ...
	  */
	  ...
	  for(std::vector<Particle>::iterator i =B_cand1.begin(); i != B_cand1.end(); i++)
	  {
		  if ((*i).child(0).child(1).mdstCharged())
			  mode = 2;
		  else
			  mode = 1;
...
*/
		  ...
		  kstarmass = (*i).child(0).p().mag();
		  TagB_tpl->column( "kstarmass"    , kstarmass         );//new
		  ...
		  for(std::vector<Mdst_ecl_aux>::iterator s = eclaux_mag.begin(); s !=eclaux_mag.end();s++ )
		  {
			  ...
			  int nfinalsig = (*i).relation().nFinalStateParticles(); // the number of final particle used to tagB
			  for(int k2 = 0 ; k2 < nfinalsig; k2++)
			  {
				  Particle childsig = (*i).relation().finalStateParticle(k2);
				  if ( childsig.mdstCharged() )
				  {
					  if ( childsig.mdstCharged().trk() == ch.trk())
					  {
						  flag=1;
					  }
				  }
				  if (childsig.mdstGamma())
				  {
					  if (childsig.mdstGamma().ecl().get_ID()==ch.get_ID())
					  {
						  flag=1;
					  }
				  }
...
/*
	    ...
	    bvertex = kvf.get_vertex();
		  ...
		  else if (mode==2)
		  {
			  ...
			  int nfinals = (*i).relation().nFinalStateParticles(); // the number of final particle used to tagB
			  for(int k11 = 0 ; k11 < nfinals; k11++)
			  {
				  Particle childs = (*i).relation().finalStateParticle(k11);
				  if ( childs.mdstCharged() )
				  {
					  ncharges ++;
				  }
			  }
			  ...
			  {
				  kvertexfitter kvfsig;
				  Mdst_charged bfinalnumbers[nfinals];
				  ExKFitterParticle btagfs[nfinals];
				  Particle bfinals[nfinals];
				  for(int k21 = 0 ; k21 < nfinals; k21++)
				  {
					  int expid = 0;
					  Particle bfinal_tmps = (*i).relation().finalStateParticle(k21);
					  bfinals[k21]=bfinal_tmps;
					  int nunp = -1 ;
					  if(!(bfinals[k21].mdstCharged()))
						  continue;
					  addTrack2fit(kvfsig,  bfinals[k21]);
 ...
 */
			  ...
			  if(MCstatus == 1)
			  {
				  ...
				  if (abs((*i).child(0).child(0).lund())==321)
				  {
					  const Mdst_charged ch11 = (*i).child(0).child(0).mdstCharged();
					  const Mdst_gamma ch121 = (*i).child(0).child(1).child(0).mdstGamma();
					  const Mdst_gamma ch122 = (*i).child(0).child(1).child(1).mdstGamma();
					  ...
					  Gen_hepevt Evtch121 = get_hepevt(ch121);
					  TagB_tpl->column("hi121",Evtch121.idhep());
					  Gen_hepevt EvtP121;
					  Gen_hepevt EvtGP121;
					  Gen_hepevt EvtGGP121;
					  
					  Gen_hepevt Evtch122 = get_hepevt(ch122);
					  TagB_tpl->column("hi122",Evtch122.idhep());
					  Gen_hepevt EvtP122;
					  Gen_hepevt EvtGP122;
					  Gen_hepevt EvtGGP122;
					  ...
					  if(Evtch121.mo(0))
					  {
						  EvtP121 = gen_mgr[Evtch121.mo(0)-1];
						  TagB_tpl->column("mo121",EvtP121.idhep());
						  if(EvtP121.mo(0))
						  {
							  EvtGP121 = gen_mgr[EvtP121.mo(0)-1];
							  TagB_tpl->column("gmo121",EvtGP121.idhep());
							  if(EvtGP121.mo(0))
							  {
								  EvtGGP121 = gen_mgr[EvtGP121.mo(0)-1];
								  TagB_tpl->column("ggmo121",EvtGGP121.idhep());
							  }
						  }
					  }
					  if(Evtch122.mo(0))
					  {
						  EvtP122 = gen_mgr[Evtch122.mo(0)-1];
						  TagB_tpl->column("mo122",EvtP122.idhep());
						  if(EvtP122.mo(0))
						  {
							  EvtGP122 = gen_mgr[EvtP122.mo(0)-1];
							  TagB_tpl->column("gmo122",EvtGP122.idhep());
							  if(EvtGP122.mo(0))
							  {
								  EvtGGP122 = gen_mgr[EvtGP122.mo(0)-1];
								  TagB_tpl->column("ggmo122",EvtGGP122.idhep());
							  }
						  }
					  }
					  if(Evtch11.idhep()==321&&Evtch121.idhep()==22&&Evtch122.idhep()==22&&EvtP11.idhep()==323&&EvtP121.idhep()==111 &&EvtP122.idhep()==111&&EvtGP121.idhep()==323 &&EvtGP122.idhep()==323 &&EvtGP11.idhep()==521 &&EvtGGP121.idhep()==521&&EvtGGP122.idhep()==521 )
					  {
						  hindex = 1;
					  }
					  else if(Evtch11.idhep()==-321&&Evtch121.idhep()==22&&Evtch122.idhep()==22&&EvtP11.idhep()==-323&&EvtP121.idhep()==111 &&EvtP122.idhep()==111&&EvtGP121.idhep()==-323 &&EvtGP122.idhep()==-323 &&EvtGP11.idhep()==-521 &&EvtGGP121.idhep()==-521&&EvtGGP122.idhep()==-521)
					  {
						  hindex = 2;
					  }
					  else
				  ...
				  else if ((*i).child(0).child(0).lund()==310)
				  {
					  const Mdst_charged ch111 = (*i).child(0).child(0).child(0).mdstCharged();
					  const Mdst_charged ch112 = (*i).child(0).child(0).child(1).mdstCharged();
					  const Mdst_charged ch12  = (*i).child(0).child(1).mdstCharged();
					  
					  Gen_hepevt Evtch111 = get_hepevt(ch111);
					  TagB_tpl->column("hi111",Evtch111.idhep());
					  Gen_hepevt EvtP111;
					  Gen_hepevt EvtGP111;
					  Gen_hepevt EvtGGP111;
					  Gen_hepevt EvtGGGP111;

					  Gen_hepevt Evtch112 = get_hepevt(ch112);
					  TagB_tpl->column("hi112",Evtch112.idhep());
					  Gen_hepevt EvtP112;
					  Gen_hepevt EvtGP112;
					  Gen_hepevt EvtGGP112;
					  Gen_hepevt EvtGGGP112;

					  Gen_hepevt Evtch12 = get_hepevt(ch12);
					  TagB_tpl->column("hi12",Evtch12.idhep());
					  Gen_hepevt EvtP12;
					  Gen_hepevt EvtGP12;
					  Gen_hepevt EvtGGP12;
					  Gen_hepevt EvtGGGP12;

					  if(Evtch111.mo(0))
					  {
						  EvtP111 = gen_mgr[Evtch111.mo(0)-1];
						  TagB_tpl->column("mo111",EvtP111.idhep());
						  if(EvtP111.mo(0))
						  {
							  EvtGP111 = gen_mgr[EvtP111.mo(0)-1];
							  TagB_tpl->column("gmo111",EvtGP111.idhep());
							  if(EvtGP111.mo(0))
							  {
								  EvtGGP111 = gen_mgr[EvtGP111.mo(0)-1];
								  TagB_tpl->column("ggmo111",EvtGGP111.idhep());
								  if(EvtGGP111.mo(0))
								  {
									  EvtGGGP111 = gen_mgr[EvtGGP111.mo(0)-1];
									  TagB_tpl->column("gggmo111",EvtGGGP111.idhep());
								  }
							  }
						  }
					  }
					  if(Evtch112.mo(0))
					  {
						  EvtP112 = gen_mgr[Evtch112.mo(0)-1];
						  TagB_tpl->column("mo112",EvtP112.idhep());
						  if(EvtP112.mo(0))
						  {
							  EvtGP112 = gen_mgr[EvtP112.mo(0)-1];
							  TagB_tpl->column("gmo112",EvtGP112.idhep());
							  if(EvtGP112.mo(0))
							  {
								  EvtGGP112 = gen_mgr[EvtGP112.mo(0)-1];
								  TagB_tpl->column("ggmo112",EvtGGP112.idhep());
								  if(EvtGGP112.mo(0))
								  {
									  EvtGGGP112 = gen_mgr[EvtGGP112.mo(0)-1];
									  TagB_tpl->column("gggmo112",EvtGGGP112.idhep());
								  }
							  }
						  }
					  }
					  if(Evtch12.mo(0))
					  {
						  EvtP12 = gen_mgr[Evtch12.mo(0)-1];
						  TagB_tpl->column("mo12",EvtP12.idhep());
						  if(EvtP12.mo(0))
						  {
							  EvtGP12 = gen_mgr[EvtP12.mo(0)-1];
							  TagB_tpl->column("gmo12",EvtGP12.idhep());
							  if(EvtGP12.mo(0))
							  {
								  EvtGGP12 = gen_mgr[EvtGP12.mo(0)-1];
								  TagB_tpl->column("ggmo12",EvtGGP12.idhep());
								  if(EvtGGP12.mo(0))
								  {
									  EvtGGGP12 = gen_mgr[EvtGGP12.mo(0)-1];
									  TagB_tpl->column("gggmo12",EvtGGGP12.idhep());
								  }
							  }
						  }
					  }
					  if(Evtch111.idhep()==211&&Evtch112.idhep()==-211&&Evtch12.idhep()==211&&EvtP111.idhep()==310&&EvtP112.idhep()==310&&EvtGP111.idhep()==311&&EvtGP112.idhep()==311 &&EvtP12.idhep()==323&&EvtGGP111.idhep()==323 &&EvtGGP112.idhep()==323 &&EvtGGGP111.idhep()==521 &&EvtGGGP112.idhep()==521&&EvtGP12.idhep()==521 )
					  {
						  hindex = 1;
					  }
					  else if(Evtch111.idhep()==211&&Evtch112.idhep()==-211&&Evtch12.idhep()==-211&&EvtP111.idhep()==310&&EvtP112.idhep()==310&&EvtGP111.idhep()==-311&&EvtGP112.idhep()==-311 &&EvtP12.idhep()==-323&&EvtGGP111.idhep()==-323 &&EvtGGP112.idhep()==-323 &&EvtGGGP111.idhep()==-521 &&EvtGGGP112.idhep()==-521&&EvtGP12.idhep()==-521)
					  {
						  hindex = 2;
					  }
					  else
  ...
  spher = Sper(ptl, tmpjet);
	...
	double ana_fullreconk::bestcandchi(Particle B_cand )
	{
		float vchisq;
		int DOF;
		float chisqpdef;
		int flagch = -1;
		int ncharge = 0;
		std::cout<<"start bestcandchi 1 " <<std::endl;
		
		int nfinal = B_cand.relation().nFinalStateParticles();
		std::cout<<"nfinal =  " <<nfinal<<std::endl;
		
		for(int k = 0 ; k < nfinal; k++)
		{
			std::cout<<"k =  " <<k<<std::endl;
			
			Particle child = B_cand.relation().finalStateParticle(k);
			std::cout<<"start bestcandchi 2 " <<std::endl;
		...
		if(flagch)
		{
			...
			kmassfitter kvf;
			std::cout<<"start bestcandchi 3 " <<std::endl;
			
			Particle bfinal[nfinal];
			for(int k2 = 0 ; k2 < nfinal; k2++)
			{
				std::cout<<"start bestcandchi 4 " <<std::endl;
				
				int expid = 0;
				Particle bfinal_tmp = B_cand.relation().finalStateParticle(k2);
				std::cout<<"start bestcandchi 5 " <<std::endl;
				...
				addTrack2fit(kvf,  bfinal[k2]);
				std::cout<<"start bestcandchi 6 " <<std::endl;
				
			}
			kvf.invariantMass(B_cand.pType().mass());
			std::cout<<"start bestcandchi 7 " <<std::endl;
			
			HepPoint3D origin(0.,0.,0.);
			kvf.vertex(origin);
			unsigned err = kvf.fit(); // do "fitting"
			std::cout<<"start bestcandchi 8 " <<std::endl;
			
			if(err == 0) // success.
			{
				vchisq = kvf.chisq();
				DOF = kvf.dgf();
				chisqpdef = vchisq/DOF;
			}
			else
			{
				vchisq = -1;
