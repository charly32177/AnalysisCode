	...
	void ana_fullreconk::event(BelleEvent* evptr, int* status)
	{
		...
		for( Ekpfullrecon_Manager::iterator frecon_it = frec_mgr.begin(); frecon_it != frec_mgr.end(); frecon_it++ )
		{
			...
			for(std::vector<Particle>::iterator i =B_cand1.begin(); i != B_cand1.end(); i++)
			{
				...
				nremaining = npip+npim+nep+nem+nkp+nkm+nmup+nmum+npm+npp ;
				double a = -1;
				double e1 = (*i).child(0).child(1).child(0).e();
				double e2 = (*i).child(0).child(1).child(1).e();
				a = abs(e1-e2)/(e1+e2);
				...
				for (std::vector<Gen_hepevt>::iterator i = gen_mgr.begin();i != gen_mgr.end(); i++)
				{
					...
					if (abs((*i).idhep())==521)
					{
						...
						Gen_hepevt nu;
						Gen_hepevt nubar;
						Gen_hepevt kstarchild0;
						Gen_hepevt kstarchild1;
						int kda1;
						int kda2;
						int knbpd;
						if( nbpd == 3)
						{
							kstar=gen_mgr[(*i).da(0)-1];
							std::cout<<"db1.4"<<std::endl;
							nu=gen_mgr[(*i).da(0)];
							std::cout<<"db1.5"<<std::endl;
							nubar=gen_mgr[(*i).da(0)+1];
							std::cout<<"db1.6"<<std::endl;
							kda1=(kstar).da(0),kda2=(kstar).da(1);
							knbpd = (kda1-kda2+1);
							std::cout<<"db1.66"<<std::endl;
						}
						int mcmode = -2;
						if ((*i).idhep() == 521 && mcmode!=1 && nbpd == 3)
						{
							...
							if (kstar.idhep() == 323 && nu.idhep() == 18 && nubar.idhep() == -18)
							{
								...
								if (kstar.da(0))
								{
									std::cout<<"db1.7b"<<std::endl;
									kstarchild0=gen_mgr[kstar.da(0)-1];
									std::cout<<"db1.7"<<std::endl;
									kstarchild1=gen_mgr[kstar.da(0)];
									
									if (kstarchild0.idhep() == 321 && kstarchild1.idhep() == 111 )
										mcmode = 1;
									else
										mcmode = 11;
								}
						...
						if ((*i).idhep() == -521 && mcmode!=1 && nbpd == 3)
						{
							...
							std::cout<<"db -1.91"<<std::endl;
							if (kstar.idhep() == -323 && nu.idhep() == 18 && nubar.idhep() == -18)
							{
								std::cout<<"db -1.92"<<std::endl;
								TagB_tpl->column("topcheck",-1);
								//								std::cout<< "kstarchild0 (-321)= " << kstarchild0.idhep() << "  " << "kstarchild1(111) = " << kstarchild1.idhep() << std::endl;
								if (kstar.da(0))
								{
									std::cout<<"db1.7b"<<std::endl;
									kstarchild0=gen_mgr[kstar.da(0)-1];
			...
			for(std::vector<Particle>::iterator i =B_cand2.begin(); i != B_cand2.end(); i++)
			{
				...
				for (std::vector<Gen_hepevt>::iterator i = gen_mgr.begin();i != gen_mgr.end(); i++)
				{
					if (abs((*i).idhep())==521)
					{
						...
						int mcmode = -2;
						if ((*i).idhep() == 521 && mcmode != 2 && nbpd == 3)
						{
							for(int start=0;start<(da2-da1+1);start++)
							{
								Gen_hepevt Evda=gen_mgr[(*i).da(0)-1+start];
							...
							if (kstar.idhep() == 323 && nu.idhep() == 18 && nubar.idhep() == -18)
							{
								...
								if ((kstar).da(0))
								{
									...
									if (kstarchild0.idhep() == 311 && kstarchild1.idhep() == 211 )
										mcmode=2;
									else
										mcmode=21;
								}
