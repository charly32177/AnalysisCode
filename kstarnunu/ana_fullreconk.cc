...
//		bTagB_tpl = BASF_Histogram->ntuple( "Rank1 tagB meson1","ebeam Tag_RunID Tag_EvtID Tag_ExpID tagid tagBmbc tagBde BtagE decmod ddecmod1 ddecmod2 ddecmod3 ddecmod4 mcinfo nFS best nbout csbest csnbout", 3 );
		...
		pi0_tpl1 = BASF_Histogram->ntuple("pi0mode1","pi0mass pi0hindex a cos hi0 hi1 mo0 mo1 gmo0 gmo1 pi0p pi0e", 13);
		pi0_tpl2 = BASF_Histogram->ntuple("pi0mode2","pi0mass pi0hindex a cos hi0 hi1 mo0 mo1 gmo0 gmo1 pi0p pi0e", 14);
...
/*		
				bTagB_tpl->column( "ebeam"    , ebeam          );
				bTagB_tpl->column( "Tag_RunID", Run            );
				bTagB_tpl->column( "Tag_EvtID", Evt            );
				bTagB_tpl->column( "Tag_ExpID", Exp            );
				bTagB_tpl->column( "tagid"    , frecB.tag_id() );
				bTagB_tpl->column( "tagBmbc"  , frecB.Mbc()    );
				bTagB_tpl->column( "tagBde"   , frecB.DeltaE() );
				bTagB_tpl->column( "decmod"   , frecB.decay()       );
				bTagB_tpl->column( "ddecmod1" , frecB.Ddec(0)       );
				bTagB_tpl->column( "ddecmod2" , frecB.Ddec(1)       );
				bTagB_tpl->column( "ddecmod3" , frecB.Ddec(2)       );
				bTagB_tpl->column( "ddecmod4" , frecB.Ddec(3)       );
				bTagB_tpl->column( "mcinfo"   , frecB.MCinfo()      );
				bTagB_tpl->column( "nFS"      , frecB.nFS()         );
				bTagB_tpl->column( "best"     , frecB.NBRank()      );
				bTagB_tpl->column( "nbout"    , frecB.NBout()       );
				bTagB_tpl->column( "csbest"   , frecB.cont_NBRank() );
				bTagB_tpl->column( "csnbout"  , frecB.cont_NBout()  );
				bTagB_tpl->dumpData();	
*/
			gamma.clear();
			gamma1.clear();
...
/*				pi0_tpl->column("pi0hindex",pi0hindex);
	...
	*/			
				/////////////////////////////////
				if (e1>0.05&&e2>0.05&&pi0mass>0.1178&&pi0mass<0.1502 && a < 0.9)
				{
					int flags1=-1;
					int flags2=-1;
					int flags3=-1;
			...
			for(std::vector<Particle>::iterator i =B_cand1.begin(); i != B_cand1.end(); i++)
			{
				...
				if ((*i).child(0).p().mag() > 0.8166 && (*i).child(0).p().mag() < 0.9666 )
				{
					if (nbmulti1 > 1)
					{
						for(std::vector<Particle>::iterator j2=B_cand1.begin(); j2 != B_cand1.end(); j2++)
						{
							if ((*j2).child(0).p().mag() < 0.8166 || (*j2).child(0).p().mag() > 0.9666)
								continue;
				...
				TagB_tpl->column( "masrank1"  , masrank1  );
				TagB_tpl->column( "conmrank1" , conmrank1  );
				...
				if ((*i).child(0).p().mag() > 0.8166 && (*i).child(0).p().mag() < 0.9666  )
				{
					if (nbmulti1 > 1)
					{
						for(std::vector<Particle>::iterator j2=B_cand1.begin(); j2 != B_cand1.end(); j2++)
						{
							...
							Mdst_gamma jMdst_10 =(*j2).child(0).child(1).child(0).mdstGamma();
							Mdst_gamma jMdst_11 =(*j2).child(0).child(1).child(1).mdstGamma();
							Mdst_charged jMdst_0 =(*j2).child(0).child(0).mdstCharged();
							...
							jkstar_init.setX(IP.x() + (*j2).child(0).p().px()/(*j2).child(0).p().rho());
							jkstar_init.setY(IP.y() + (*j2).child(0).p().py()/(*j2).child(0).p().rho());
							jkstar_init.setZ(IP.z() + (*j2).child(0).p().pz()/(*j2).child(0).p().rho());
							ExKFitterVertex jkstar_Vertex(jkstar_init);
							ExKFitterVertex jB_Vertex(IP,IPerr);
				...
				TagB_tpl->column( "chirank1"  , chirank1  );
				TagB_tpl->column( "concrank1"  , concrank1  );
				...
				TagB_tpl->column( "n_ks"    , n_tks         );//new
				...
				int mcmode = -2;
				std::vector<Gen_hepevt> bchild;
				std::vector<Gen_hepevt> kstarchild;
				bchild.clear();
				kstarchild.clear();
				for (std::vector<Gen_hepevt>::iterator i = gen_mgr.begin();i != gen_mgr.end(); i++)
				{
					...
					if (abs((*i).idhep())==521)
					{
						...
						if ((*i).idhep() == 521 )
						{
							std::cout<<"db1.8"<<std::endl;
							for(int start=0;start<(da2-da1+1);start++)
							{
								Gen_hepevt Evda=gen_mgr[(*i).da(0)-1+start];
								bchild.push_back(Evda);
								std::cout<<"db1.9"<<std::endl;
								char bpdnumber[32];
								sprintf (bpdnumber,"%s%d","bpd",start+1);
								TagB_tpl->column(bpdnumber,Evda.idhep());
							}
							std::cout<<"hi bitch"<<std::endl;
							//									std::cout<< "kstarchild0 = " << kstarchild0.idhep() << "  " << "kstarchild1 = " << kstarchild1.idhep() << std::endl;
							if (bchild[0].da(0))
							{
								int kda1=bchild[0].da(0),kda2=bchild[0].da(1);
								for(int start=0;start<(kda2-kda1+1);start++)
								{
									Gen_hepevt Evda=gen_mgr[bchild[0].da(0)-1+start];
									kstarchild.push_back(Evda);
									std::cout<<"db1.983"<<std::endl;
									char bpdnumber[32];
									sprintf (bpdnumber,"%s%d","kspd",start+1);
									TagB_tpl->column(bpdnumber,Evda.idhep());
								}
								if (kstarchild[0].idhep() == 321 && kstarchild[1].idhep() == 111 && bchild[0].idhep() == 323 && bchild[1].idhep() == 18 && bchild[2].idhep() == -18 )
								{
									mcmode = 1;
						...
						if ((*i).idhep() == -521 )
						{
							...
							for(int start=0;start<(da2-da1+1);start++)
							{
								Gen_hepevt Evda=gen_mgr[(*i).da(0)-1+start];
								bchild.push_back(Evda);
								std::cout<<"db-1.9"<<std::endl;
								char bndnumber[32];
								sprintf (bndnumber,"%s%d","bnd",start+1);
							...
							if (bchild[0].da(0))
							{
								...
								for(int start=0;start<(kda2-kda1+1);start++)
								{
									...
									std::cout<<"db1.983"<<std::endl;
									char bndnumber[32];
									sprintf (bndnumber,"%s%d","ksnd",start+1);
									TagB_tpl->column(bndnumber,Evda.idhep());
								}
								if (kstarchild[0].idhep() == -321 && kstarchild[1].idhep() == 111 && bchild[0].idhep() == -323 && bchild[1].idhep() == 18 && bchild[2].idhep() == -18)
								{
						...
						TagB_tpl->column("mcmode",mcmode);
					}
					
				}
				
				for(std::vector<Particle>::iterator i1 = tpi_0.begin(); i1 != tpi_0.end(); i1++)
				{
					...
					e2 = (*i1).child(1).e();
					a = abs(e1-e2)/(e1+e2);
					cos= costheta((*i1).child(0),(*i1).child(1));
					Mdst_pi0 pi0yo = (*i1).mdstPi0();
					double pi0mass = pi0yo.mass();
					if(MCstatus == 1)
					{
						const Mdst_gamma ch0 = (*i1).child(0).mdstGamma();
						const Mdst_gamma ch1 = (*i1).child(1).mdstGamma();
						Gen_hepevt Evtch0 = get_hepevt(ch0);
						//	pi0_tpl->column("hi0",Evtch0.idhep());
						Gen_hepevt Evtch1 = get_hepevt(ch1);
						//	pi0_tpl->column("hi1",Evtch1.idhep());
					 ...
					 if (frecB.Mbc() >5.24&&abs(frecB.DeltaE())<0.05&&sum_ecl<2&&tnremaining==1&&n_tks==0&&log(frecB.cont_NBout())>-7&& (*i).child(0).p().mag()>0.8166&&(*i).child(0).p().mag()<0.9666&&concrank1==1&&mcmode==1)
					 {
						 pi0_tpl1->column("pi0hindex",pi0hindex);
						 pi0_tpl1->column("pi0mass",pi0mass);
						 pi0_tpl1->column("a",a);
						 pi0_tpl1->column("cos",cos);
						 pi0_tpl1->column("pi0p",pmag(*i1));
						 pi0_tpl1->column("pi0e",(*i1).e());
						 pi0_tpl1->dumpData();
					 }
				}//end pi_0
			...
			for(std::vector<Particle>::iterator i =B_cand2.begin(); i != B_cand2.end(); i++)
			{
				...
				if ( (*i).child(0).p().mag() > 0.8166 && (*i).child(0).p().mag() < 0.9666 )
				{
					if (nbmulti2 > 1)
					{
						for(std::vector<Particle>::iterator j2=B_cand2.begin(); j2 != B_cand2.end(); j2++)
						{
							if (i == j2)
								continue;
				...
				TagB_tpl2->column( "conmrank2"  , conmrank2  );
				TagB_tpl2->column( "kstrank2"  , kstrank2  );
				TagB_tpl2->column( "masrank2"  , masrank2  );
				...
				if ( (*i).child(0).p().mag() > 0.8166 && (*i).child(0).p().mag() < 0.9666 )
				{
					if (nbmulti2 > 1)
					{
						for(std::vector<Particle>::iterator j2=B_cand2.begin(); j2 != B_cand2.end(); j2++)
						{
							...
							double chi1;
							double chi2;
							Mdst_charged jMdst_00 =(*j2).child(0).child(0).child(0).mdstCharged();
							Mdst_charged jMdst_01 =(*j2).child(0).child(0).child(1).mdstCharged();
							Mdst_charged jMdst_1 =(*j2).child(0).child(1).mdstCharged();
							...
							jks_init.setX(IP.x() + (*j2).child(0).child(0).p().px()/(*j2).child(0).child(0).p().rho());
							jks_init.setY(IP.y() + (*j2).child(0).child(0).p().py()/(*j2).child(0).child(0).p().rho());
							jks_init.setZ(IP.z() + (*j2).child(0).child(0).p().pz()/(*j2).child(0).child(0).p().rho());
							ExKFitterVertex jks_Vertex(jks_init);
							ExKFitterVertex jB_Vertex(IP,IPerr);
				...
				TagB_tpl2->column("chirank2",chirank2);
				TagB_tpl2->column("concrank2",concrank2);
				...
				TagB_tpl2->column( "n_ks"    , n_tks         );//new
				...
				for (std::vector<Gen_hepevt>::iterator i = gen_mgr.begin();i != gen_mgr.end(); i++)
				{
					...
					if (abs((*i).idhep())==521)
					{
						std::cout<<"db1.1"<<std::endl;
						int da1=(*i).da(0),da2=(*i).da(1);
						std::cout<<"db1.2"<<std::endl;
						...
						if ((*i).idhep() == 521)
						{
							...
							if (bchild[0].da(0))
							{
								...
								for(int start=0;start<(kda2-kda1+1);start++)
								{
									...
									std::cout<<"db1.983"<<std::endl;
									char bndnumber[32];
									sprintf (bndnumber,"%s%d","kspd",start+1);
									TagB_tpl2->column(bndnumber,Evda.idhep());
								}
								if (kstarchild[0].idhep() == 311 && kstarchild[1].idhep() == 211 && bchild[0].idhep() == 323 && bchild[1].idhep() == 18 && bchild[2].idhep() == -18)
								{
									mcmode = 2;
				...
				TagB_tpl2->column("mcmode",mcmode);
				
				for(std::vector<Particle>::iterator i1 = pi_0.begin(); i1 != pi_0.end(); i1++)
				{
					...
					if (frecB.Mbc() >5.24&&abs(frecB.DeltaE())<0.05&&sum_ecl<2&&tnremaining==1&&n_tks==1&&log(frecB.cont_NBout())>-7&& (*i).child(0).p().mag()>0.8166&&(*i).child(0).p().mag()<0.9666&&concrank2==1&&mcmode==2)
					{
						pi0_tpl2->column("pi0hindex",pi0hindex);
						pi0_tpl2->column("pi0mass",pi0mass);
						pi0_tpl2->column("a",a);
						pi0_tpl2->column("cos",cos);
						pi0_tpl2->column("pi0p",pmag(*i1));
						pi0_tpl2->column("pi0e",(*i1).e());
						pi0_tpl2->dumpData();
					}
				}//end pi_0
